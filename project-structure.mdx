...
- **Framer Motion** — the full animation library (version 11, as identified by the `version: "11.18.2"` string in the bundle)
...
<AccordionGroup>
  ...
  <Accordion title="SparkleCursor.js — Sparkle Particle Effect">
    Tracks `mousemove` events on `window` and spawns short-lived `✦` glyphs at the cursor position. Each sparkle is assigned a random size (5–15px), one of four colors (`#FF69B4`, `#00CED1`, `#FFFACD`, `#FFFFFF`), and a random rotation. Particles fade out via CSS `opacity` transition after 800ms. Respects `prefers-reduced-motion`:
  ...
  <Accordion title="VisitorCounter.js, WebringBadges.js, UnderConstructionSign.js">
    **`WebringBadges`** renders a horizontal row of `Button88x31` instances representing fictional or real webring memberships — a staple of late-1990s personal pages.

    **`UnderConstructionSign`** renders an animated "Under Construction" notice, typically placed on pages that are not yet complete, matching the era's tradition of displaying animated GIF construction signs.
  </Accordion>
